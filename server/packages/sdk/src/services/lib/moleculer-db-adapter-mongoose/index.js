...
exports.MongooseDbAdapter = void 0;
const lodash_1 = __importDefault(require('lodash'));
const moleculer_1 = require('moleculer');
const mongoose_1 = __importDefault(require('mongoose'));
const ServiceSchemaError = moleculer_1.Errors.ServiceSchemaError;
class MongooseDbAdapter {
  ...
  connect() {
    ...
    if (this.model) {
      ...
      if (mongoose_1.default.connection.readyState == 1) {
        ...
        conn = mongoose_1.default.connection.asPromise();
      } else {
        conn = mongoose_1.default
          .connect(this.uri, this.opts)
          .then((m) => m.connection);
      }
    } else if (this.schema) {
      conn = mongoose_1.default
  ...
  createCursor(params) {
    if (params) {
      ...
      // Search
      if (lodash_1.default.isString(params.search) && params.search !== '') {
      ...
      // Sort
      if (lodash_1.default.isString(params.sort))
        q.sort(params.sort.replace(/,/, ' '));
      else if (Array.isArray(params.sort)) q.sort(params.sort.join(' '));
      // Offset
      if (lodash_1.default.isNumber(params.offset) && params.offset > 0)
        q.skip(params.offset);
      // Limit
      if (lodash_1.default.isNumber(params.limit) && params.limit > 0)
  ...
  beforeSaveTransformID(entity, idField) {
    const newEntity = lodash_1.default.cloneDeep(entity);
  ...
  stringToObjectID(id) {
    if (typeof id == 'string' && mongoose_1.default.Types.ObjectId.isValid(id))
      return new mongoose_1.default.Schema.Types.ObjectId(id);
    return id;
  }
  /**
...
exports.MongooseDbAdapter = MongooseDbAdapter;
