...
exports.ApiGatewayMixin = void 0;
...
const { MoleculerError, MoleculerServerError, ServiceNotFoundError } =
  moleculer_1.Errors;
const match = moleculer_1.Utils.match;
...
exports.ApiGatewayMixin = {
        ...
        this.aliases.forEach((alias) => {
          ...
          if (withActionSchema && alias.action) {
            ...
            if (actionSchema && actionSchema.action) {
              obj.action = lodash_1.default.omit(actionSchema.action, [
                'handler',
              ]);
            }
          }
  ...
  methods: {
    ...
    createServer() {
      ...
      if (
        ...
        this.server = this.settings.http2
          ...
          : https_1.default.createServer(this.settings.https, this.httpHandler);
        this.isHTTPS = true;
      } else {
        this.server = this.settings.http2
          ? http2_1.default.createServer(this.httpHandler)
          : http_1.default.createServer(this.httpHandler);
        this.isHTTPS = false;
      }
      // HTTP server timeout
    ...
    errorHandler(req, res, err) {
      ...
      if (
        ...
        (err && !lodash_1.default.inRange(err.code, 400, 500))
      ) {
        this.logger.error(
          '   Request error!',
          err.name,
    ...
    async httpHandler(req, res, next) {
      ...
      if (this.settings.rootCallOptions) {
        if (lodash_1.default.isPlainObject(this.settings.rootCallOptions)) {
          Object.assign(options, this.settings.rootCallOptions);
        } else if (lodash_1.default.isFunction(this.settings.rootCallOptions)) {
    ...
    routeHandler(ctx, route, req, res, foundAlias) {
      ...
      return new this.Promise(async (resolve, reject) => {
        ...
        try {
          await utils_1.composeThen.call(this, req, res, ...route.middlewares);
          let params = {};
          ...
          if (route.opts.mergeParams === false) {
            ...
            const body = lodash_1.default.isObject(req.body) ? req.body : {};
          ...
          if (route.opts.camelCaseNames) {
            action = action
              .split('.')
              .map(lodash_1.default.camelCase)
              .join('.');
          }
          // Alias handler
    ...
    async aliasHandler(req, res, alias) {
      ...
      if (route.rateLimit) {
        ...
        if (key) {
          ...
          if (remaining < 0) {
            throw new errors_1.RateLimitExceeded();
          }
        }
      }
      ...
      if (lodash_1.default.isFunction(alias.handler)) {
        ...
        if (alias.action)
          ...
          throw new MoleculerServerError(
            ...
            'NO_ALIAS_HANDLER',
            {
              ...
              alias: lodash_1.default.pick(alias, ['method', 'path']),
            }
          );
    ...
    sendResponse(req, res, data, action) {
      ...
      // Buffer from Object
      else if (lodash_1.default.isObject(data) && data.type == 'Buffer') {
      ...
      // Stream
      else if ((0, isstream_1.isReadable)(data)) {
        res.setHeader(
          'Content-Type',
          responseType || 'application/octet-stream'
        );
      ...
      else if (lodash_1.default.isObject(data) || Array.isArray(data)) {
        res.setHeader(
          'Content-Type',
      ...
      // Other (stringify or raw text)
        ...
        if (!responseType) {
          ...
          if (lodash_1.default.isString(data)) chunk = data;
      ...
      if (
        ...
        chunk &&
        !res.getHeader('ETag') &&
        !(0, isstream_1.isReadable)(chunk)
      ) {
        res.setHeader(
          'ETag',
          utils_1.generateETag.call(this, chunk, route.etag)
        );
      }
      // Freshness
      if ((0, utils_1.isFresh)(req, res)) res.statusCode = 304;
      ...
      if (req.method === 'HEAD') {
        ...
        // respond
        if ((0, isstream_1.isReadable)(data)) {
          //Stream response
          data.pipe(res);
        } else {
          res.end(chunk);
    ...
    send404(req, res) {
      ...
      this.sendError(req, res, new errors_1.NotFoundError());
    },
    /**
    ...
    sendError(req, res, err) {
      // Route error handler
      if (req.$route && lodash_1.default.isFunction(req.$route.onError))
        return req.$route.onError.call(this, req, res, err);
      // Global error handler
      if (lodash_1.default.isFunction(this.settings.onError))
      ...
      res.setHeader('Content-type', responseType);
      const code =
        lodash_1.default.isNumber(err.code) &&
        lodash_1.default.inRange(err.code, 400, 599)
          ? err.code
          : 500;
      res.writeHead(code);
    ...
    reformatError(err /*, req, res*/) {
      return lodash_1.default.pick(err, [
        'name',
        'message',
        'code',
        'type',
    ...
    parseQueryString(req) {
      ...
      if (questionIdx !== -1) {
        query = qs_1.default.parse(req.url.substring(questionIdx + 1));
    ...
    coloringStatusCode(code) {
      if (code >= 500) return kleur_1.default.red().bold(code);
      if (code >= 400 && code < 500) return kleur_1.default.red().bold(code);
      if (code >= 300 && code < 400) return kleur_1.default.cyan().bold(code);
      if (code >= 200 && code < 300) return kleur_1.default.green().bold(code);
      /* istanbul ignore next */
      return code;
    },
    /**
    ...
    logResponse(req, res, data) {
      ...
      if (req.$startTime) {
        ...
        if (duration > 1000)
          time = kleur_1.default.red(
            `[+${Number(duration / 1000).toFixed(3)} s]`
          );
        else
          time = kleur_1.default.grey(`[+${Number(duration).toFixed(3)} ms]`);
      }
      if (this.settings.logResponse && this.settings.logResponse in this.logger)
        this.logger[this.settings.logResponse](
          `<= ${this.coloringStatusCode(res.statusCode)} ${
            req.method
          } ${kleur_1.default.bold(req.originalUrl)} ${time}`
        );
      /* istanbul ignore next */
      if (
    ...
    checkOrigin(origin, settings) {
      if (lodash_1.default.isString(settings)) {
        ...
        if (settings.indexOf('*') !== -1) {
          ...
          const wildcard = new RegExp(
            `^${lodash_1.default
              .escapeRegExp(settings)
              .replace(/\\\*/g, '.*')
              .replace(/\\\?/g, '.')}$`
          );
          return origin.match(wildcard);
        }
      } else if (lodash_1.default.isFunction(settings)) {
    ...
    writeCorsHeaders(route, req, res, isPreFlight) {
      ...
      if (!route.cors.origin || route.cors.origin === '*') {
        ...
        throw new errors_1.ForbiddenError(errors_1.ERR_ORIGIN_NOT_ALLOWED);
      }
      ...
      if (lodash_1.default.isString(route.cors.exposedHeaders)) {
        res.setHeader(
          'Access-Control-Expose-Headers',
      ...
      if (isPreFlight) {
        ...
        if (lodash_1.default.isString(route.cors.allowedHeaders)) {
          res.setHeader(
            'Access-Control-Allow-Headers',
        ...
        if (lodash_1.default.isString(route.cors.methods)) {
    ...
    checkWhitelist(route, action) {
      ...
      return (
        route.whitelist.find((mask) => {
          if (lodash_1.default.isString(mask)) return match(action, mask);
          else if (lodash_1.default.isRegExp(mask)) return mask.test(action);
        }) != null
      );
    },
    /**
    ...
    optimizeRouteOrder() {
      this.routes.sort((a, b) => {
        let c =
          (0, utils_1.addSlashes)(b.path).split('/').length -
          (0, utils_1.addSlashes)(a.path).split('/').length;
        if (c == 0) {
    ...
    createRoute(opts) {
      ...
      if (opts.authorization) {
        ...
        if (lodash_1.default.isString(opts.authorization))
      ...
      if (opts.authentication) {
        ...
        if (lodash_1.default.isString(opts.authentication))
      ...
      if (opts.bodyParsers) {
        ...
        Object.keys(bps).forEach((key) => {
          const opts = lodash_1.default.isObject(bps[key])
            ? bps[key]
            : undefined;
          if (bps[key] !== false && key in body_parser_1.default)
            route.middlewares.push(body_parser_1.default[key](opts));
        });
      }
      // Logging
      ...
      if (rateLimit) {
        ...
        if (opts.StoreFactory) {
          ...
          route.rateLimit.store = new memory_store_1.MemoryStore(opts.window);
        }
      }
      // Handle whitelist
      ...
      const globalPath =
        this.settings.path && this.settings.path != '/'
          ? this.settings.path
          : '';
      route.path = (0, utils_1.addSlashes)(globalPath) + (opts.path || '');
      route.path = (0, utils_1.normalizePath)(route.path);
      // Create aliases
      ...
      if (!route.mappingPolicy) {
        const hasAliases =
          lodash_1.default.isObject(opts.aliases) &&
    ...
    createRouteAliases(route, aliases) {
      ...
      lodash_1.default.forIn(aliases, (action, matchPath) => {
    ...
    regenerateAutoAliases(route) {
      ...
      services.forEach((service) => {
        ...
        let basePaths = [];
        if (lodash_1.default.isString(service.settings.rest)) {
          basePaths = [service.settings.rest];
        } else if (lodash_1.default.isArray(service.settings.rest)) {
          basePaths = service.settings.rest;
        } else {
        ...
        for (let basePath of basePaths) {
          basePath = (0, utils_1.addSlashes)(
            lodash_1.default.isString(basePath)
              ? basePath
          ...
          lodash_1.default.forIn(service.actions, (action) => {
            if (action.rest) {
              ...
              if (
                ...
                return;
              let restRoutes = [];
              if (!lodash_1.default.isArray(action.rest)) {
              ...
              for (const restRoute of restRoutes) {
                let alias = null;
                if (lodash_1.default.isString(restRoute)) {
                  alias = this.parseActionRestString(restRoute, basePath);
                } else if (lodash_1.default.isObject(restRoute)) {
                ...
                if (alias) {
                  alias.path = (0, utils_1.removeTrailingSlashes)(
                    (0, utils_1.normalizePath)(alias.path)
                  );
    ...
    createAlias(route, path, action) {
      const alias = new alias_1.Alias(this, route, path, action);
  ...
  created() {
    if (this.settings.server !== false) {
      if (lodash_1.default.isObject(this.settings.server)) {
    ...
    if (this.settings.assets) {
      ...
      this.serve = (0, serve_static_1.default)(
        this.settings.assets.folder,
        opts
      );
    }
    // Alias store
    ...
    const debounceTime =
      this.settings.debounceTime > 0
        ...
        : 500;
    this.regenerateAllAutoAliases = lodash_1.default.debounce(() => {
      /* istanbul ignore next */
      this.routes.forEach(
  ...
  started() {
    ...
    return new this.Promise((resolve, reject) => {
      this.server.listen(this.settings.port, this.settings.ip, (err) => {
        ...
        const listenAddr =
          addr.address == '0.0.0.0' && os_1.default.platform() == 'win32'
            ? 'localhost'
            : addr.address;
        this.logger.info(
  ...
  serveStatic: serve_static_1.default,
