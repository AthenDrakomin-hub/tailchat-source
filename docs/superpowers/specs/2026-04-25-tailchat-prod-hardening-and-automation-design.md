...
- `SOCKETIO_CORS_ORIGINS`：Socket.IO 允许的 origin 列表（默认 `https://goodspage.cn`）
...
1. 生产机 `IMAGE_TAG=... bash scripts/pull-up.sh` 不再因为短暂 404/502 误判失败（探活最终可稳定通过）
2. Socket.IO 仅允许 `https://goodspage.cn` 连接（可通过 env 扩展）
